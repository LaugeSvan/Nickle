...
async function createWeeklyOnsdagshyggePost() {
    try {
        ...
        if (!guild) {
            console.error('error.guild.notfound.Botten er ikke i nogen guilds.');
            return;
        }
        ...
        if (!channel) {
            console.error(`error.channel.notfound.Kunne ikke finde Forum-kanal ved navn #${TARGET_CHANNEL_NAME} i guild: ${guild.name}.`);
            return;
        }
